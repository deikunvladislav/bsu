...
public:
    ...
    void create_with_employees(const std::vector<Employee>& employees) {
        ...
        if (!ofs) {
            throw std::runtime_error("Cannot create file: " + path_);
        }
    ...
    bool update_by_id(const Employee& updated) {
        ...
        if (!fs) {
            throw std::runtime_error("Cannot open file for update: " + path_);
        }
        Employee e{};
...
private:
    ...
    static bool read_record(std::istream& s, Employee& e) {
        if (!s.read(reinterpret_cast<char*>(&e.num), sizeof(e.num))) {
            return false;
        }
