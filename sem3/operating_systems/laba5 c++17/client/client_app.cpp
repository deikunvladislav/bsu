...
static bool recv(NamedPipeClient& pipe, Proto::Response& resp) {
    if (!pipe.receive_response(resp)) {
...
int run_client() {
    try {
        ...
        Proto::Response probe{};
        if (pipe.receive_response(probe)) {
            if (!probe.ok) {
                if (std::string(probe.message) == "Shutdown") {
                    std::cout << "[Client] Server is shutting down.\n";
                    return 0;
                }
                std::cerr << "[Client] " << probe.message << '\n';
                wait_enter("Press Enter to exit...");
                return 1;
            }
            else {
                std::cout << "[Client] " << probe.message << '\n';
            }
        }
        else {
        ...
        while (true) {
            ...
            if (opt == 1) {
                ...
                const double hours = Console::prompt_double("New hours: ");
                resp.payload.set_name(name);
                resp.payload.hours = hours;
